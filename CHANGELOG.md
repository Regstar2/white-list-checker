...
- выбор системной, светлой и тёмной темы;
...
- главный экран теперь содержит восемь симметрично расположенных разделов;
- удалён лишний заголовок «Быстрые действия».
- Переработан раздел «Уведомления»: главный экран стал коротким обзором локального и Telegram-каналов.
- Локальные и Telegram-уведомления разделены на отдельные экраны.
- Настройка Telegram Worker, поиск получателей и очередь сообщений вынесены во внутренние экраны.
- Получатели Telegram отображаются компактными строками с overflow-меню вместо постоянной кнопки удаления.
- Добавлена иерархическая Back-навигация внутри раздела уведомлений.

## 0.10.4 — Statistics UI, Diagnostics UI, Localization & Local Export
...
- Добавлен полноценный английский перевод основных экранов приложения и переключение языка в настройках.
- Добавлен раздел со скриншотами в `README.md` и `README_EN.md`; изображения перенесены в `docs/assets/screenshots/`.
...
- Экран «Диагностика» переработан в компактную Material 3 структуру с главным результатом, сводкой, раскрываемыми группами сайтов, мониторингом состояния и progressive disclosure технической статистики.
- Статусы `WhitelistState`, ошибки сайтов и часть сообщений Telegram/Public Service теперь отображаются через локализованные пользовательские строки.
- `README.md` восстановлен как русская UTF-8 версия и синхронизирован с `README_EN.md` по структуре, статусам и ссылкам.
...
- Исправлен краш при запуске после переключения локализованного Compose `LocalContext`: `ActivityResultRegistryOwner` теперь явно сохраняется в composition locals.
- В диагностике больше не показываются внутренние значения `SiteCheckErrorType.name` как пользовательский текст.
